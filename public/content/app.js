...
$(document).ready(function () {
  ...
  $(
    ".sizeInput, .arrivalInput, .firstCpu, .inOut, .lastCpu, .quantumIn, .fixedPart, .inputMemory"
  ).keydown(function (e) {
    if (
  ...
  $(".optionOne").click(function () {
    ...
    maxpart = 5;
    $(".textoAlertMem").text(
      "5 es la cantidad maxima de particiones para el tamaño de memoria elegida."
    );
    console.log(sizeMemory);
  });
  $(".optionTwo").click(function () {
    ...
    maxpart = 8;
    $(".textoAlertMem").text(
      "8 es la cantidad maxima de particiones para el tamaño de memoria elegida."
    );
    console.log(sizeMemory);
  });
  $(".optionThree").click(function () {
    ...
    maxpart = 12;
    $(".textoAlertMem").text(
      "12 es la cantidad maxima de particiones para el tamaño de memoria elegida."
    );
    console.log(sizeMemory);
  });
  //--------------------------------
  ...
  $(".sizeInput").keyup(function () {
    ...
    if (tamProc > maxTamPocess) {
      $(".textoAlertProc").text(
        "El tamaño del proceso no puede ser mayor al tamaño de la Memoria o Particion."
      );
      $(".alertProcess").addClass("show");
    }
  });
  ...
  $(".startButton").click(function () {
    ...
    //  }
    $("#proccessBar")
      .attr("aria-valuenow", firstIrruption)
      .css("width", firstIrruption + "%");
    ...
    var htmlPopover =
      ...
      "</div>";
    htmlPopover +=
      "<div>Tiempo de Ejecucion: " + arrayCpu[0].irrupctionTime + "</div>";
    if (arrayCpu[0].finish) {
      ...
      var markupFirst =
        ...
        arrayCpu[0].outTime +
        "</td><td>" +
        arrayCpu[0].arrivalTime +
        "</td><td>" +
        (arrayCpu[0].outTime - arrayCpu[0].arrivalTime) +
        "</td></tr>";
    ...
    for (var i = 1; i < arrayCpu.length; i++) {
      ...
      newItem
        .attr("aria-valuenow", item.irruption)
        .css("width", irruption + "%");
      var tag = newItem.find("a");
      var htmlTag =
        "<div>Desde " + item.inTime + " hasta " + item.outTime + "</div>";
      ...
      if (arrayCpu[i].name == "O") {
        ...
        htmlTag +=
          "<div>Tiempo de Ejecucion: " + item.irrupctionTime + "</div>";
      }
      ...
      if (item.finish) {
        ...
        var markWait =
          ...
          irruptionTot +
          "</td><td>" +
          (item.outTime - item.arrivalTime - irruptionTot) +
          "</td></tr>";
    ...
    $(".timeReturn").text(
      "Tiempo de Retorno Promedio: " + (totalTime / totalProcess).toFixed(2)
    );
    $(".timeWait").text(
      "Tiempo de Espera Promedio: " + (totalWait / totalProcess).toFixed(2)
    );
    ...
    //}
    $("#proccessEs")
      .attr("aria-valuenow", firstIrruptionEs)
      .css("width", firstIrruptionEs + "%");
    ...
    if (arrayEs[0].name == "O") {
      ...
      if (firstIrruptionEs < 2) {
        ...
        tagOneEs
          .css("background-color", arrayEs[0].color)
          .text(arrayEs[0].name);
      }
    ...
    for (var i = 1; i < arrayEs.length; i++) {
      ...
      var htmlTag =
        "<div><b>De " + item.inTime + " a " + item.outTime + "</b></div>";
  ...
  if (sizepart > 0) {
    if (sizepart <= tamdisp) {
      ...
      if (cont < maxpart) {
        ...
        controlForm
          .find(".entry:not(:last) .inputMemory")
        ...
        controlForm
          ...
          .addClass("btn-danger")
          .html(
            '<span class="glyphicon glyphicon-minus deleteInput">Quitar</span>'
          );
      } else {
        controlForm
          .find(".entry:last .inputMemory")
          .addClass("classDisabled")
          .removeClass("inputMemory")
          .prop("disabled", true);

        controlForm
          .find(".entry:last .btn-add")
          .removeClass("btn-add")
          .addClass("btn-remove")
          .removeClass("btn-success")
          .addClass("btn-danger")
      ...
      $(".textoAlertPart").text(
        "El tamaño de la partición es mayor al disponible."
      );
      $(".alertPart").addClass("show");
    }
  } else {
...
function graficarMem(particiones, procName) {
  ...
  barraProg.empty();
  barraProg.append(
    '<div class="progress-bar itemMem tt" data-toggle="tooltip" title="P1" data-placement="top"  role="progressbar" style="width: 100%;" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100">F</div>'
  );
  ...
  if (particiones[0].used == "") {
    itemProg
      ...
      .removeClass("bg-info")
      .addClass("bg-success")
      .attr("title", "Libre: " + particiones[0].size + "KB.");
    ...
    if (particiones[0].used == procName) {
      itemProg
        ...
        .attr(
          "title",
          particiones[0].used + ": " + particiones[0].size + "KB."
        );
    } else {
      itemProg
        .text("P")
  ...
  for (var i = 1; i < particiones.length; i++) {
    ...
    if (particiones[i].used == "") {
      ...
      newBar
        ...
        .removeClass("bg-info")
        .addClass("bg-success")
        .attr("title", "Libre: " + particiones[i].size + "KB.");
    } else {
      if (particiones[i].used == procName) {
        newBar
          ...
          .attr(
            "title",
            particiones[i].used + ": " + particiones[i].size + "KB."
          );
      } else {
        newBar
          .text("P")
...
$(document).on("click", ".btn-add-raf", function (e) {
  ...
  if (raf < maxraf) {
    ...
    if (cpu > 0 && es > 0) {
      ...
      $(this)
        .parents(".entryRaf:first")
        .addClass("divDelete" + raf);

      controlForm
        .find(".entryRaf:not(:last) .inputcpu")
        .addClass("classDisabled")
        .removeClass("inputcpu")
        .prop("disabled", true);

      controlForm
        .find(".entryRaf:not(:last) .inputes")
        .addClass("classDisabled")
        .removeClass("inputes")
        .prop("disabled", true);

      controlForm
        .find(".entryRaf:not(:last) .btn-add-raf")
        .removeClass("btn-add-raf")
        .addClass("btn-remove-raf")
        .removeClass("btn-success")
        .addClass("btn-danger")
...
function saveData() {
  ...
  if (
    ...
    if (size > maxTamPocess) {
      $(".textoAlertProc").text(
        "El tamaño del proceso no puede ser mayor al tamaño de memoria."
      );
      $(".alertProcess").addClass("show");
    } else {
    ...
    .then((querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        for (var i = 0; i < doc.data().cpuTime.length; i++) {
          tiempos =
            tiempos + doc.data().cpuTime[i] + "-" + doc.data().ioTime[i] + "-";
        }
        ...
        tabla.innerHTML += `
            <tr>
                ...
                <td class="tdTable"><button class="btn btn-danger" onclick="deleteData('${
                  doc.id
                }')">Borrar</button></td>
            </tr>
            `;
...
function obtenerTiempoMax() {
  ...
  for (var i = 0; i < procesos.length; i++) {
    ...
    for (var j = 0; j < procesos[i].cpuTime.length; j++) {
      tiempoMax +=
        parseInt(procesos[i].ioTime[j]) + parseInt(procesos[i].cpuTime[j]);
    }
  }
  return tiempoMax;
}
  ...
  for (j = 0; j < controladorBucle; j++) {
    ...
    for (var x = 0; x < colaListo.length; x++) {
      ...
      if (colaListo[x].cpuTime.length == 0 && colaListo[x].ioTime.length == 0) {
        if (
          colaListo[x].lastCpuTime > 0 &&
          estaEn(colaCPU, colaListo[x]) == false &&
          enCPU != colaListo[x]
        ) {
          colaCPU.push(colaListo[x]);
        }
  ...
  for (j = 0; j < controladorBucle; j++) {
    ...
    if (enCPU != null) {
      ...
      if (enCPU.cpuTime[0] > 0) {
        enCPU.cpuTime[0] -= 1;
        //colaListo[indice].cpuTime-=1;
        if (enCPU.cpuTime[0] < 1 || t1 == quantum) {
          elementoCPU.outTime = elementoCPU.inTime + elementoCPU.irrupctionTime;
          if (estaEn(colaListo, enCPU) == true && enCPU.cpuTime[0] < 1) {
            posicion = getIxByName(colaListo, enCPU.name);
        ...
        enCPU.lastCpuTime -= 1;
        //colaListo[indice].lastCpuTime-=1;
        if (enCPU.lastCpuTime < 1 || t1 == quantum) {
          elementoCPU.outTime = elementoCPU.inTime + elementoCPU.irrupctionTime;
          if (enCPU.lastCpuTime == 0) {
            elementoCPU.finish = true;
            p = getIxByName(colaListo, enCPU.name);
  ...
  for (i = 0; i < controladorBucle; i++) {
    ...
    if (enCPU != null) {
      ...
      if (enCPU.cpuTime[0] > 0) {
        ...
        enCPU.lastCpuTime -= 1; //trata tiempo de lastCpuTime
  ...
  for (i = 0; i < controladorBucle; i++) {
    ...
    if (flag == true) {
      if (enCPU.cpuTime[0] > 0) {
        ...
        if (enCPU.cpuTime[0] > newProcess[0].cpuTime[0]) {
          ...
          if (enCPU.cpuTime[0] < 1) {
            elementoCPU.outTime =
              elementoCPU.inTime + elementoCPU.irrupctionTime;
            if (estaEn(colaListo, enCPU) == true) {
              posicion = getIxByName(colaListo, enCPU.name);
              colaListo[posicion].cpuTime.splice(0, 1);
            }
    ...
    if (enCPU != null) {
      ...
      if (enCPU.cpuTime[0] > 0) {
        ...
        enCPU.lastCpuTime -= 1; //tratra tiempos de lastCpuTime
